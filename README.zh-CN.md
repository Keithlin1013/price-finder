...
对每个结果，TypeSafe 回答 6 个是非题：

1. 标题里写了品牌吗？（没写品牌就跳过第 2 项，交给系列判断）
2. 和你要的是同一品牌吗？
3. 同一系列吗？（每个修饰词都算：Platinum 和 Platinum Plus 是不同系列）
4. 香型、口味兼容吗？（标题没写香型不算不同）
5. 形态一样吗？（液体、粉末、凝胶、洗碗块等；你没写形态就不限）
6. 是同一类东西吗？（洗衣液、洗手液等）

**"只看同款"时 6 项都要通过。容量和件数始终不检查**，因为结果按单价排。判断结果按"查询 + 商品标题"存在 `.cache/verdicts/`，同一商品每次答案一样；判断规则一改，旧答案自动作废、重新判断。

### 两种比法：只看同款 / 包括相似

- **只看同款**：上面 5 项全部通过才算。适合具体的商品，比如 "Tide Ultra Concentrated Original 170 fl oz"。
- **包括相似**：只要求是**同一类东西**（都是洗衣液、都是洗手液）；你写了品牌就限定品牌，写了香型就限定香型，没写的不管。适合宽泛的搜索，比如 "Tide laundry detergent"、"hand soap refill"。

TypeSafe 会先判断你的输入是指"一件具体商品"还是"一类东西"，自动选比法（条码一律按同款）。结果标题右边有「只看同款 / 包括相似」切换按钮，切换时不重新访问网站，只用已经读到的结果重新组合判断。
